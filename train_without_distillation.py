...
# Transforming labels to one-hot encoded
...
filepath = "train_without_distillation.h5"
...
my_validation_batch_generator = My_Generator(np.array(valid['Path']), Y_valid, batch_size, shuffle)
...
with open('train_without_distillation_hist.json', 'w') as f:
    json.dump(hist.history, f)

# serialize model to JSON
model_json = teacher.to_json()
with open("train_without_distillation.json", "w") as json_file:
    json_file.write(model_json)

# load json and create model
json_file = open('train_without_distillation.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("train_without_distillation.h5")
